...
def test_filter_clients_1(clients):
    ...
    assert utility.filter_inv(clients[0], "john doe") == clients[0], "Failed to find included name"

def test_filter_clients_2(clients):
    """Success case: filter_clients() finds 0 entries by name"""
    assert utility.filter_clients(clients[0], "someone else") == [], "Wrongly found excluded name"
...
def test_filter_clients_4(clients):
    """Success case: filter_clients() finds 0 entries by valid phone number"""
    assert utility.filter_clients(clients[0], "wrong number") == [], "Wrongly found excluded phone number"

def test_filter_clients_5(clients):
    """Success case: filter_clients() finds 0 entry by invalid name"""
    assert utility.filter_clients(clients[0], "") == [], "Wrongly searched using invalid name"

def test_filter_clients_6(clients):
...
def test_filter_clients_fail_5():
    """Failure case: filter_clients() throws IndexError if clients[entry][1] does not exist"""
    try:
        utility.filter_clients([["1",]], 0)
        assert False, "Did not throw IndexError for clients[entry][1] does not exist"
    except IndexError:
        assert True
